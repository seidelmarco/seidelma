...
import this
import sys
#from dataproject.models import NewEntryWatchlist
...
'''
tickers = list()
for line in NewEntryWatchlist.objects.all():
    ticker = line.ticker
...
for i in tickers:
    ...
    try:
        # Bsp. Verlust -$15.14 und -R6 und -€0.33
        if kd['EPS'][0] == '-':
            eps = float(kd['EPS'][2:])
            eps = eps*-1
        else:
    ...
    cur.execute('''INSERT OR IGNORE INTO dataproject_keydata
                ...
                peratio, dividend, eps, divyield, exdivdate, shortrate, lower_52, upper_52,
                url)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
    ...
    cur.execute('''UPDATE dataproject_keydata SET price=?, change=?, changepercent=?,
                ...
                exdivdate=?, shortrate=?, lower_52=?, upper_52=?, url=? WHERE ticker=?''',
